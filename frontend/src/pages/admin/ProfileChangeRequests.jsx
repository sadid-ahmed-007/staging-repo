...
export default function ProfileChangeRequests() {
 ...
 <h1 className="mt-2 text-2xl font-bold text-[var(--text-primary)] sm:text-3xl">Profile Change Requests</h1>
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">
 Review and manage user profile change requests
 {pendingCount > 0 && (
 <span className="ml-2 inline-flex items-center rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-medium text-amber-800 /30 ">
 ...
 <p className="font-medium text-[var(--text-primary)] ">{req.user_name}</p>
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">{req.user_email}</p>
 </td>
 <td className="py-0 pr-4">
 <Badge variant="secondary">{roleLabels[req.user_role] || req.user_role}</Badge>
 </td>
 <td className="py-0 pr-4 font-medium text-[var(--text-primary)] ">{req.field_label}</td>
 <td className="py-0 pr-4">
 <p className="text-xs">
 <span className="text-[var(--text-muted)] line-through">{req.current_value || 'N/A'}</span>
 <span className="mx-1">→</span>
 <span className="font-medium text-primary-600 ">{req.requested_value}</span>
 </p>
 </td>
 <td className="py-0 pr-4">
 <Badge variant={statusVariants[req.status]}>{req.status.toUpperCase()}</Badge>
 </td>
 <td className="py-0 pr-4 text-xs text-[var(--text-muted)]">{formatDate(req.created_at)}</td>
 <td className="py-0">
 <button
 ...
 <div>
 <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)]">User</p>
 <p className="mt-1 text-sm font-medium text-[var(--text-primary)] ">{selectedRequest.user_name}</p>
 <p className="text-xs text-[var(--text-muted)]">{selectedRequest.user_email}</p>
 </div>
 <div>
 <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)]">Role</p>
 <p className="mt-1">
 <Badge variant="secondary">{roleLabels[selectedRequest.user_role] || selectedRequest.user_role}</Badge>
 </p>
 </div>
 <div>
 <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)]">Field</p>
 <p className="mt-1 text-sm font-medium text-[var(--text-primary)] ">{selectedRequest.field_label}</p>
 </div>
 <div>
 <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)]">Status</p>
 <p className="mt-1">
 ...
 <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 /40">
 <div className="grid grid-cols-2 gap-4">
 <div>
 <p className="text-xs font-semibold uppercase text-[var(--text-muted)]">Current Value</p>
 <p className="mt-1 text-sm text-[var(--text-secondary)] ">{selectedRequest.current_value || 'N/A'}</p>
 </div>
 <div>
 <p className="text-xs font-semibold uppercase text-primary-500">Requested Value</p>
 <p className="mt-1 text-sm font-semibold text-primary-700 ">{selectedRequest.requested_value}</p>
 </div>
 </div>
 </div>

 <div>
 <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)]">Reason</p>
 <p className="mt-1 text-sm text-[var(--text-secondary)] italic">"{selectedRequest.reason}"</p>
 </div>

 {/* Documents */}
 {selectedRequest.documents && selectedRequest.documents.length > 0 && (
 <div>
 <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)] mb-2">
 ...
 className="flex items-center gap-2 rounded-lg border border-gray-200 bg-[var(--bg-surface)] p-3 text-sm text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/20"
 >
 ...
 ? 'bg-green-50 border-green-200 /20 /30'
 : 'bg-red-50 border-red-200 /20 /30'
 }`}>
 <p className="text-xs font-semibold uppercase text-[var(--text-muted)] mb-1">Admin Notes</p>
 <p className={`text-sm ${
 ...
 <div className="space-y-4">
 <div className="rounded-xl bg-red-50 p-4 /20">
 ...
 <textarea
 className="w-full rounded-xl border border-gray-300 bg-[var(--bg-surface)] px-4 py-2 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 min-h-[100px]"
 value={rejectNotes}
