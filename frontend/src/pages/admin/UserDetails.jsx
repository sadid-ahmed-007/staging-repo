...
function InfoRow({ icon: Icon, label, value }) {
 ...
 {Icon && <Icon className="mt-0.5 h-4 w-4 flex-shrink-0 text-[var(--text-muted)]" />}
 <span className="w-40 flex-shrink-0 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">{label}</span>
 <span className="text-sm font-medium text-[var(--text-primary)] break-words">{String(value)}</span>
 </div>
 );
}
...
function StatBox({ label, value, color = 'primary', onClick }) {
 ...
 <Component 
 onClick={onClick}
 className={`rounded-xl border border-gray-100 p-4 text-center ${onClick ? 'hover:border-primary-300 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition cursor-pointer w-full focus:outline-none' : ''}`}
 >
 <p className={`text-2xl font-bold text-${color}-600 dark:text-${color}-400`}>{value ?? 0}</p>
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] mt-1">{label}</p>
 </Component>
 );
}
...
export default function AdminUserDetails() {
 ...
 setUser((u) => ({ ...u, is_deactivated: false, deactivated_at: null }));
 } catch (err) {
 ...
 className="inline-flex items-center gap-2 text-sm font-medium text-[var(--text-secondary)] dark:text-[var(--text-muted)] hover:text-primary-600 transition"
 >
 ...
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)] mt-0.5">{user.email}</p>
 ...
 {user.is_deactivated && (
 <Button variant="success" onClick={handleReactivate} loading={actionLoading}>
 ...
 {/* Warning Banner */}
 {user.suspended_at && (
 <div className="rounded-xl border border-red-200 bg-red-50 p-4 /40 /20">
 ...
 {user.is_deactivated && !user.suspended_at && (
 <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 /40 /20">
 <div className="flex items-start gap-3">
 ...
 This account was deactivated on {user.deactivated_at ? new Date(user.deactivated_at).toLocaleDateString() : 'unknown date'}. Use the Reactivate button above to restore access.
 </p>
 </div>
 </div>
 </div>
 ...
 : 'text-[var(--text-muted)] hover:text-[var(--text-secondary)] dark:text-[var(--text-muted)] dark:hover:text-gray-200'
 }`}
 >
 ...
 <p className="text-sm text-[var(--text-secondary)] dark:text-[var(--text-muted)]">This will permanently remove the user and their data.</p>
 <textarea
 value={rejectReason}
 ...
 <div className="space-y-4">
 <p className="text-sm text-[var(--text-secondary)] dark:text-[var(--text-muted)]">This will suspend the user, preventing them from logging in.</p>
 <textarea
 value={suspendReason}
 ...
 <div className="space-y-4">
 <p className="text-sm text-[var(--text-secondary)] dark:text-[var(--text-muted)]">Please provide a reason for revoking this certificate.</p>
 <textarea
...
function OverviewTab({ user, onStatClick, onRevokeGrant }) {
 ...
 <span className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)] block mb-1.5">
 NID / Birth Certificate (Full)
 </span>
 {isLegacy ? (
 <span className="text-sm italic text-[var(--text-muted)] dark:text-[var(--text-muted)]">{value}</span>
 ) : (
 <div className="rounded-lg border border-amber-200 bg-amber-50 /40 /20 px-3 py-2">
 <p className="text-[10px] font-semibold uppercase tracking-wider text-amber-600 mb-1">
 Sensitive — Admin Only
 </p>
 <span className="font-mono text-sm font-semibold text-amber-900 tracking-wider">
 {value}
 </span>
 </div>
 )}
 ...
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">No profile data available.</p>
 )}
 </Card>
 ...
 <Card>
 <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Current Enrollment</h2>
 {user.enrollments_summary.filter((e) => e.status === 'active').map((e) => (
 <div key={e.id} className="rounded-xl border border-gray-100 p-4 space-y-2 mt-2">
 <div className="flex items-center justify-between">
 <p className="font-medium text-[var(--text-primary)] ">{e.institution}</p>
 <Badge variant="success">Active</Badge>
 </div>
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">{e.program} · Batch {e.batch} · Student ID: {e.roll_number || '—'}</p>
 <p className="text-xs text-[var(--text-muted)]">Enrolled: {e.enrollment_date} · Expected: {e.expected_grad || 'TBD'}</p>
 </div>
 ))}
 </Card>
 )}
 ...
 <h2 className="text-lg font-semibold text-[var(--text-primary)] ">Active Access Grants</h2>
 </div>
 <Badge variant="primary">{accessGrants.length}</Badge>
 </div>
 {accessGrants.length === 0 ? (
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">No active access grants for this student.</p>
 ) : (
 <div className="space-y-3">
 {accessGrants?.map((grant) => (
 <div key={grant.id} className="rounded-xl border border-gray-100 p-4 space-y-2.5">
 ...
 <div className="flex flex-wrap items-center gap-3 pt-1 text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] border-t border-gray-100 ">
 ...
 <h2 className="text-lg font-semibold text-[var(--text-primary)] ">Students with Active Access</h2>
 </div>
 <Badge variant="primary">{accessGrants.length}</Badge>
 </div>
 {accessGrants.length === 0 ? (
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">This verifier currently has no active student access.</p>
 ) : (
 <div className="space-y-3">
 {accessGrants?.map((grant) => (
 <div key={grant.id} className="rounded-xl border border-gray-100 p-4 space-y-2.5">
 ...
 {user.stats && (
 <Card>
 <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Statistics</h2>
...
function AccessGrantsTab({ user, onRevokeGrant }) {
 ...
 <ShieldCheck className="h-10 w-10 text-gray-300 dark:text-[var(--text-secondary)] mb-3" />
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">No active access grants for this student.</p>
 <p className="text-xs text-[var(--text-muted)] mt-1">When verifiers are granted access to certificates, they will appear here.</p>
 </div>
 ) : (
 ...
 <th key={h} className="py-3 px-3 text-left text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)]">
 {h}
 </th>
 ))}
 </tr>
 ...
 <td className="px-3 py-2 font-medium text-[var(--text-primary)] text-xs">
 ...
 <td className="px-3 py-2 text-[var(--text-muted)] dark:text-[var(--text-muted)] text-xs font-mono">
 {grant.verifierEmail || grant.verifier_email || '—'}
 </td>
 <td className="px-3 py-2 text-[var(--text-muted)] dark:text-[var(--text-muted)] text-xs whitespace-nowrap">
 ...
 {grants.length === 0 ? (
 <div className="flex flex-col items-center py-12 text-center">
 <UserCheck className="h-10 w-10 text-gray-300 dark:text-[var(--text-secondary)] mb-3" />
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">This verifier currently has no active student access.</p>
 </div>
 ) : (
 <div className="overflow-x-auto -mx-4 sm:mx-0">
 ...
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">{history.length} total records</p>
 </div>
 <div className="overflow-x-auto -mx-4 sm:mx-0">
 <table className="w-full min-w-[700px] text-sm">
 <thead>
...
function CertificatesTab({ certificates, loading, total, currentPage, lastPage, onPageChange, onViewDetails, role, filter, onFilterChange }) {
 ...
 className="rounded-lg border border-[var(--border)] bg-[var(--bg-surface)] px-2 py-1 text-sm text-[var(--text-primary)]"
 >
 <option value="all">All Statuses</option>
 <option value="active">Active</option>
 <option value="revoked">Revoked</option>
 </select>
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">{total} total</p>
 </div>
 </div>

 {filteredCertificates.length === 0 ? (
 <div className="flex flex-col items-center py-12 text-center">
 <FileText className="h-10 w-10 text-gray-300 dark:text-[var(--text-secondary)] mb-3" />
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">No certificates found.</p>
 </div>
 ) : (
 <>
 <div className="overflow-x-auto -mx-4 sm:mx-0">
 ...
 <td className="px-3 py-0 font-mono text-xs text-[var(--text-primary)] ">{c.serial}</td>
 <td className="px-3 py-0 text-[var(--text-secondary)] text-xs">{c.certificate_name}</td>
 <td className="px-3 py-0 text-[var(--text-muted)] dark:text-[var(--text-muted)] text-xs">{role === 'university' ? c.student_name : c.institution_name}</td>
 <td className="px-3 py-0"><Badge variant="default">{c.certificate_level}</Badge></td>
 <td className="px-3 py-0 text-[var(--text-secondary)] text-xs">{c.cgpa || '—'}</td>
 <td className="px-3 py-0 text-[var(--text-muted)] dark:text-[var(--text-muted)] text-xs whitespace-nowrap">{c.issue_date}</td>
 ...
 className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1 text-xs font-medium text-[var(--text-secondary)] hover:border-primary-300 hover:text-primary-600 transition"
 >
...
function EnrollmentsTab({ enrollments, loading, total, currentPage, lastPage, onPageChange, role, filter, onFilterChange }) {
 ...
 <Card>
 <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
 <h2 className="text-lg font-semibold text-[var(--text-primary)] ">Enrollments</h2>
 <div className="flex items-center gap-3">
 <select 
 value={filter} 
 onChange={(e) => onFilterChange(e.target.value)}
 ...
 <div className="flex flex-col items-center py-12 text-center">
 <BookOpen className="h-10 w-10 text-gray-300 dark:text-[var(--text-secondary)] mb-3" />
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">No enrollments found.</p>
 </div>
 ) : (
 <>
 <div className="overflow-x-auto -mx-4 sm:mx-0">
 ...
 <td className="px-3 py-0 text-sm text-[var(--text-primary)] ">{role === 'university' ? e.student_name : e.institution_name}</td>
 <td className="px-3 py-0 text-xs text-[var(--text-secondary)] ">{e.roll_number || '—'}</td>
 <td className="px-3 py-0 text-xs text-[var(--text-secondary)] ">{e.program}</td>
 <td className="px-3 py-0 text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">{e.batch || '—'}</td>
 <td className="px-3 py-0 text-xs font-mono text-[var(--text-muted)] dark:text-[var(--text-muted)]">{e.enrollment_number || '—'}</td>
 <td className="px-3 py-0"><Badge variant={STATUS_COLORS[e.status] || 'default'}>{e.status}</Badge></td>
 <td className="px-3 py-0 text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] whitespace-nowrap">{e.enrollment_date || '—'}</td>
 <td className="px-3 py-0 text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] whitespace-nowrap">{e.expected_grad || '—'}</td>
 </tr>
 ))}
 </tbody>
 </table>
...
function ActivityTab({ activities, loading, total, currentPage, lastPage, onPageChange }) {
 ...
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">{total} entries</p>
 </div>

 {activities.length === 0 ? (
 <div className="flex flex-col items-center py-12 text-center">
 <Activity className="h-10 w-10 text-gray-300 dark:text-[var(--text-secondary)] mb-3" />
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">No activity recorded yet.</p>
 </div>
 ) : (
 <>
 <div className="space-y-3">
 {activities?.map((a) => (
 <div key={a.id} className="flex items-start gap-3 rounded-xl border border-gray-100 p-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
...
function PaginationBar({ currentPage, lastPage, onPageChange }) {
 ...
 <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Page {currentPage} of {lastPage}</p>
...
function CertificateDetailContent({ cert, onViewStudent, onRestoreClick, onRevokeClick }) {
 ...
 className="inline-flex items-center gap-1.5 rounded-lg border border-green-300 bg-green-50 /20 px-3 py-1.5 text-xs font-medium text-green-700 hover:bg-green-100 dark:hover:bg-green-900/40 transition"
 >
 ...
 className="inline-flex items-center gap-1.5 rounded-lg border border-red-300 bg-red-50 /20 px-3 py-1.5 text-xs font-medium text-red-700 hover:bg-red-100 dark:hover:bg-red-900/40 transition"
 >
 ...
 <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 /40">
 ...
 {cert.revoked_at && (
 <div className="mt-3 rounded-lg bg-red-50 /20 p-3 border border-red-100 /30">
 <p className="text-xs text-red-800 font-medium mb-1">Revocation Details</p>
 ...
 <p className="mt-4 text-sm uppercase tracking-[0.18em] text-[var(--text-muted)] dark:text-[var(--text-muted)]">Serial Number</p>
 <div className="flex items-center gap-3 mt-1">
 <p className="font-mono text-lg font-semibold text-[var(--text-primary)] ">{cert.serial}</p>
 <button
 onClick={() => handleCopySerial(cert.serial)}
 className="p-1.5 rounded-lg text-[var(--text-muted)] hover:text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition flex items-center gap-1"
 ...
 <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-stretch rounded-2xl border border-gray-200 p-5 bg-[var(--bg-surface)] /40">
 <div className="flex-1 space-y-4 w-full">
 <div>
 <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)] dark:text-[var(--text-muted)]">Verification Link</h3>
 <p className="mt-1 text-sm text-[var(--text-secondary)] ">
 Use this link or the QR code to verify this certificate's authenticity.
 </p>
 <div className="mt-3 rounded-lg bg-gray-50 /80 p-3 border border-gray-100 break-all text-sm font-mono text-[var(--text-primary)] ">
 ...
 <div className="flex items-center text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">
 <ShieldCheck className="mr-1.5 h-4 w-4" />
 <span>
 Verified <span className="font-semibold text-[var(--text-primary)] ">{cert.verification_count}</span> times 
 ...
 <div className="shrink-0 flex flex-col items-center justify-center rounded-xl border border-gray-200 bg-[var(--bg-surface)] p-3 shadow-sm ">
 <QRCodeSVG
...
function DetailTile({ label, value, action }) {
 return (
 <div className="rounded-2xl border border-gray-200 bg-[var(--bg-surface)] px-4 py-3 /60 flex flex-col justify-between">
 <div>
 <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)] dark:text-[var(--text-muted)]">{label}</p>
 <p className="mt-1 text-sm font-medium text-[var(--text-primary)] truncate" title={value || 'N/A'}>{value || 'N/A'}</p>
 </div>
 {action && (
 <button
