...
export default function Settings() {
 ...
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Manage your account, notifications, and preferences</p>
 </div>
 <div className="flex items-center gap-3">
 {saveStatus === 'saving' && (
 <span className="flex items-center gap-1.5 text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] animate-pulse">
 ...
 : 'border-transparent text-[var(--text-muted)] hover:border-gray-300 hover:text-[var(--text-secondary)] dark:text-[var(--text-muted)] dark:hover:border-gray-600 dark:hover:text-gray-300'
 }
 `}
 >
 ...
 <div className="flex items-center justify-end gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 /50">
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">Changes to preferences are not saved automatically.</p>
 ...
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Customize how the application looks.</p>
 ...
 ? 'border-primary-500 bg-primary-50 text-primary-700 /20 shadow-sm'
 : 'border-gray-200 bg-[var(--bg-surface)] text-[var(--text-secondary)] hover:border-gray-300 dark:text-[var(--text-muted)] dark:hover:border-gray-600'
 }`}
 >
 ...
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Choose which emails you receive.</p>
 ...
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Control what shows up in your notification panel.</p>
 <div className="mt-4 divide-y divide-gray-100 dark:divide-gray-800">
 ...
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Control who can see your information.</p>
 <div className="mt-4 divide-y divide-gray-100 dark:divide-gray-800">
 <div className="py-4">
 <h3 className="text-sm font-medium text-[var(--text-primary)] mb-3">Profile Visibility</h3>
 <RadioGroup
 ...
 <div className="py-4">
 <h3 className="text-sm font-medium text-[var(--text-primary)] mb-3">Default Certificate Visibility</h3>
 <RadioGroup
 ...
 />
 <p className="mt-1.5 text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">
 ...
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Recent account activity and security events.</p>
 {activitiesLoading ? (
 <div className="mt-6 flex min-h-[20vh] items-center justify-center">
 <LoadingSpinner />
 </div>
 ) : activities.length === 0 ? (
 <p className="mt-6 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">No account activity yet.</p>
 ) : (
 <>
 <div className="mt-6 overflow-x-auto">
 <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
 <thead>
 <tr className="text-left text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)]">
 ...
 <tr key={activity.id}>
 <td className="py-3 pr-4 font-medium text-[var(--text-primary)] ">{activity.action}</td>
 <td className="py-3 pr-4 text-[var(--text-secondary)] ">{activity.description}</td>
 <td className="py-3 pr-4 text-[var(--text-secondary)] ">{formatDate(activity.date)}</td>
 <td className="py-3 text-[var(--text-secondary)] ">{activity.ip_address || '-'}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">Page {page} of {totalPages}</p>
 ...
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">These actions are irreversible. Proceed with caution.</p>
 ...
 <p className="mt-0.5 text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">Permanently delete your account and all data. This requires admin approval.</p>
 </div>
...
function InfoRow({ label, value }) {
 return (
 <div className="rounded-lg bg-gray-50 p-3 /50">
 <p className="text-xs font-medium text-[var(--text-muted)] dark:text-[var(--text-muted)]">{label}</p>
...
function RadioGroup({ value, onChange, options, name }) {
 ...
 <label
 key={opt.value}
 className="flex cursor-pointer items-center gap-3 rounded-lg border-2 border-gray-200 hover:border-gray-300 dark:hover:border-gray-600 p-3 transition"
 >
 <input
 type="radio"
 name={name}
 ...
 {opt.desc && <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">{opt.desc}</p>}
 </div>
 </label>
 ))}
 </div>
...
function StudentPreferences({ draft, updateDraft }) {
 ...
 <div className="py-3">
 <h3 className="text-sm font-medium text-[var(--text-primary)] mb-1">Default Certificate Visibility</h3>
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] mb-3">Visibility applied to newly issued certificates</p>
 <RadioGroup
 ...
 <div className="py-3">
 <h3 className="text-sm font-medium text-[var(--text-primary)] mb-1">Default Access Duration</h3>
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] mb-3">How long verifiers can access your certificates by default</p>
 <RadioGroup
...
function UniversityPreferences({ draft, updateDraft, authorityDraft, setAuthorityDraft }) {
 ...
 <label className="block text-sm font-medium text-[var(--text-primary)] ">Student ID Format</label>
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">
 Template for auto-generated IDs. Use <code className="rounded bg-gray-100 px-1 ">{'{{YEAR}}'}</code> and <code className="rounded bg-gray-100 px-1 ">{'{{SEQ}}'}</code> as placeholders — e.g. <span className="font-mono">UIU-{'{{YEAR}}'}-{'{{SEQ}}'}</span>
 </p>
 <input
 type="text"
 ...
 className="mt-1 block w-full rounded-lg border border-gray-300 bg-[var(--bg-surface)] px-3 py-2 text-sm text-[var(--text-primary)] shadow-sm transition focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 "
 />
 </div>

 <div className="py-3 space-y-1">
 <label className="block text-sm font-medium text-[var(--text-primary)] ">Student ID Prefix</label>
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">Short prefix for auto-generated IDs (e.g., STU, AIUB)</p>
 <input
 type="text"
...
function ProfilePrivacyBanner({ hidden }) {
 ...
 return (
 <div className="mb-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm /50 /20">
