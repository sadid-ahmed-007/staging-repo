...
export default function Enrollments() {
 ...
 <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 transition-transform group-hover:scale-105 /40 ">
 ...
 >
 <div className="flex items-center gap-4 min-w-0">
 <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600 transition-transform group-hover:scale-105 /40 ">
 ...
 >
 <div className="flex items-center gap-4 min-w-0">
 <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-rose-50 text-rose-600 transition-transform group-hover:scale-105 /40 ">
 ...
 >
 <div className="flex items-center gap-4 min-w-0">
 <div className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-amber-50 text-amber-600 transition-transform group-hover:scale-105 /40 ">
...
function EnrollStudentModal({ isOpen, onClose, onSuccess }) {
 const [step, setStep] = useState(1); // 1 = select student, 2 = fill form
 const [searchTab, setSearchTab] = useState('search'); // 'search' or 'applications'
 ...
 // Fetch program structure and accepted applications on modal open
 useEffect(() => {
 if (isOpen) {
 ...
 setLoadingApps(true);
 api.get('/university/applications', { params: { status: 'accepted', size: 100 } })
 .then(res => {
 // Filter to only show applications that are not yet enrolled
 // (Assuming the backend returns them, or we just show them if backend doesn't filter.
 // The prompt says: "Shows list of accepted applications not yet converted to enrollment")
 ...
 const handleSelectStudent = (student, prefillData = null) => {
 setSelectedStudent(student);
 if (prefillData) {
 // Calculate grad date based on level
 let duration = 4;
 const lvl = programStructure.find(l => l.id == prefillData.certificateLevelId);
 if (lvl) duration = lvl.durationYears || 4;
 
 const newGradDate = calculateGraduationDate(formData.enrollmentDate, duration);
 
 setFormData(prev => ({
 ...prev,
 certificateLevelId: prefillData.certificateLevelId || '',
 departmentId: prefillData.departmentId || '',
 programId: prefillData.programId || '',
 program: prefillData.programName || '',
 ...
 <button
 className={cn("px-4 py-2 text-sm font-medium border-b-2", searchTab === 'search' ? 'border-[var(--brand)] text-[var(--brand)]' : 'border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]')}
 onClick={() => setSearchTab('search')}
 >
 Search Students
 </button>
 <button
 className={cn("px-4 py-2 text-sm font-medium border-b-2", searchTab === 'applications' ? 'border-[var(--brand)] text-[var(--brand)]' : 'border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]')}
 onClick={() => setSearchTab('applications')}
 >
 From Applications
 </button>
 </div>
 ...
 {/* Search Results List */}
 <div className="mt-4 max-h-[320px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
 {isSearching ? (
 ...
 <>
 <p className="text-xs text-[var(--text-secondary)]">
 Select a student from your accepted applications to pre-fill the enrollment form.
 </p>
 <div className="mt-4 max-h-[320px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
 {loadingApps ? (
 <div className="flex flex-col items-center justify-center py-8">
 ...
 <div key={app.id} className="flex items-center justify-between rounded-xl border border-[var(--border)] p-3.5 hover:border-[var(--brand)] hover:bg-[var(--bg-elevated)]/50 transition-all">
 <div>
 <h4 className="text-sm font-semibold text-[var(--text-primary)]">{app.studentName}</h4>
 <p className="text-xs text-[var(--text-secondary)]">{app.studentEmail}</p>
 <p className="text-xs text-green-600 mt-1">Accepted: {new Date(app.reviewedAt || app.appliedAt).toLocaleDateString()}</p>
 </div>
 <div className="text-right">
 <p className="text-xs font-medium text-[var(--text-primary)] mb-2 max-w-[150px] truncate">{app.programName || app.departmentName || 'General'}</p>
 <Button size="sm" onClick={() => handleSelectStudent({id: app.studentId, name: app.studentName, email: app.studentEmail}, app)}>
 Enroll
 </Button>
 </div>
 </div>
 ...
 <div className="sm:col-span-2">
 <label className="mb-1 block text-xs font-semibold text-[var(--text-secondary)]">
 Program (Certificate Name) <span className="text-rose-500">*</span>
 </label>
 <select
 required
...
function ViewEnrollmentModal({ enrollment, onClose, onEdit, onExtend, onWithdraw, onRespond, onViewCertificate }) {
 ...
 <div className="rounded-xl border border-amber-300 bg-amber-50/60 p-4 /40 /20">
 ...
 ? 'bg-slate-100/80 /40 border-b border-[var(--border)]'
 : 'bg-amber-50/80 /30 border-b border-amber-200/60 /30'
 }`}>
 <AlertTriangle className={`h-4 w-4 shrink-0 ${
 enrollment.withdrawalRequest.requestedBy === 'university'
 ? 'text-slate-500'
 ...
 : 'bg-amber-100 text-amber-700 /30 '
 }`}>
 ...
 <blockquote className="rounded-lg border-l-4 border-amber-400 bg-amber-50/50 /20 /40 px-3 py-2 text-sm italic text-[var(--text-secondary)] leading-relaxed">
 "{enrollment.withdrawalRequest.reason}"
 </blockquote>
 </div>
 )}
 ...
 enrollment.withdrawalRequest.status === 'rejected'
 ? 'border-green-400 bg-green-50/50 /20 /40'
 : 'border-slate-300 bg-slate-50/50 /30 /40'
 }`}>
 {enrollment.withdrawalRequest.responseMessage}
 </blockquote>
 </div>
 ...
 <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-4 /40 /20">
 <div className="flex items-center gap-2 text-slate-600 ">
 <LogOut className="h-4 w-4 shrink-0" />
 <h4 className="text-xs font-bold uppercase tracking-wider">Withdrawal Details</h4>
 <span className="ml-auto inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold bg-slate-100 text-slate-700 ">
 Direct Withdrawal
 </span>
 </div>
 ...
 <blockquote className="mt-2 rounded-lg border-l-4 border-slate-300 bg-[var(--bg-surface)]/50 /30 px-3 py-2 text-xs italic text-[var(--text-secondary)]">
 "{enrollment.withdrawalReason}"
 </blockquote>
 )}
 </div>
...
function ExtendGraduationModal({ enrollment, onClose, onSuccess }) {
 ...
 reason.trim().length < 10 ? 'text-rose-500' : 'text-emerald-600 '
 )}
 >
...
function DirectWithdrawModal({ enrollment, onClose, onSuccess }) {
 ...
 <form onSubmit={handleSubmit} className="space-y-4">
 {/* Warning Banner */}
 <div className="rounded-xl border border-rose-300 bg-rose-50/70 p-3.5 text-xs text-rose-700 /40 /30 ">
 ...
 className={cn(
 'text-[11px] font-medium',
 reason.trim().length < 20 ? 'text-rose-500' : 'text-emerald-600 '
 )}
 >
...
function RespondWithdrawalModal({ enrollment, onClose, onSuccess }) {
 ...
 <div className="rounded-lg border border-amber-200/60 bg-amber-50/50 p-2.5 /30 /20">
 ...
 decision === 'approve'
 ? 'border-emerald-500 bg-emerald-50 text-emerald-700 /40 shadow-sm'
 ...
 'flex items-center justify-center gap-2 rounded-xl border-2 py-3 px-4 text-xs font-bold transition-all',
 decision === 'reject'
 ? 'border-rose-500 bg-rose-50 text-rose-700 /40 shadow-sm'
 ...
 className={cn(
 'text-[11px] font-medium',
 responseMessage.trim().length < 10 ? 'text-rose-500' : 'text-emerald-600 '
 )}
 >
