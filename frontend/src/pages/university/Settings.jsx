...
export default function UniversitySettings() {
 ...
 <DashboardLayout>
 <div className="mb-8">
 <h1 className="text-2xl font-bold text-[var(--text-primary)] ">University Management</h1>
 <p className="text-[var(--text-secondary)] dark:text-[var(--text-muted)]">Configure global settings and auto-populating data.</p>
 </div>
 ...
 : 'border-transparent text-[var(--text-muted)] hover:border-gray-300 hover:text-[var(--text-secondary)] dark:text-[var(--text-muted)] dark:hover:border-gray-700 dark:hover:text-gray-300'
 }`}
 >
...
function AuthoritySettings() {
 ...
 <h3 className="text-lg font-medium text-[var(--text-primary)] ">Certificate Authority Defaults</h3>
 ...
 {currentSaved.name || <span className="italic text-[var(--text-muted)] font-normal">Not configured</span>}
 </p>
 </div>
 <div>
 <span className="text-xs text-[var(--text-muted)]">Default Authority Title:</span>
 <p className="font-semibold text-[var(--text-primary)] mt-0.5">
 {currentSaved.title || <span className="italic text-[var(--text-muted)] font-normal">Not configured</span>}
 </p>
 </div>
 </div>
 </div>
...
function CertificateLevels() {
 ...
 <h3 className="text-lg font-medium text-[var(--text-primary)] ">Certificate Levels</h3>
 {!loading && <span className="rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-[var(--text-secondary)] ">{levels.length}</span>}
 </div>
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Manage the types of certificates your institution issues.</p>
 </div>
 <Button onClick={() => openModal()}>Add Level</Button>
 </div>
 <div className="divide-y divide-gray-200 dark:divide-gray-800">
 {levels?.map((level) => (
 <div key={level.id} className={`flex items-center justify-between px-6 py-4 transition-colors ${!level.isActive ? 'opacity-60 bg-gray-50 /40' : ''}`}>
 <div>
 <div className="flex items-center gap-2">
 <h4 className={`font-medium ${!level.isActive ? 'text-[var(--text-muted)] dark:text-[var(--text-muted)] line-through' : 'text-[var(--text-primary)] '}`}>{level.name}</h4>
 ...
 {level.student_count !== undefined && <span className="ml-3 border-l border-gray-300 pl-3">{level.student_count} active students</span>}
 </p>
 </div>
 ...
 {levels.length === 0 && (
 <div className="p-8 text-center text-[var(--text-muted)]">No certificate levels found. Add one to get started.</div>
 )}
 </div>
 </Card>
...
function Departments() {
 ...
 <Card>
 <div className="border-b border-gray-200 px-6 py-5 ">
 <h3 className="text-lg font-medium text-[var(--text-primary)] ">Departments</h3>
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Manage academic departments in your institution.</p>
 
 <div className="mt-6 flex flex-col sm:flex-row gap-4 items-end">
 <div className="flex-1 max-w-sm">
 <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)] ">Select Certificate Level</label>
 <select 
 className="input-field block w-full rounded-xl border border-gray-200 bg-[var(--bg-surface)] px-4 py-2.5 text-sm text-[var(--text-primary)] focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 " 
 ...
 <div className="p-8 text-center text-[var(--text-muted)]">No departments found in this level.</div>
 ) : (
 items?.map((item) => (
 <div key={item.id} className={`flex items-center justify-between px-6 py-4 transition-colors ${!item.isActive ? 'opacity-60 bg-gray-50 /40' : ''}`}>
 <div>
 <div className="flex items-center gap-2">
 <h4 className={`font-medium ${!item.isActive ? 'text-[var(--text-muted)] dark:text-[var(--text-muted)] line-through' : 'text-[var(--text-primary)] '}`}>{item.name}</h4>
 ...
 {item.shortCode && item.student_count !== undefined && <span className="mx-3 border-l border-gray-300 "></span>}
...
function Majors() {
 ...
 <Card>
 <div className="border-b border-gray-200 px-6 py-5 ">
 <h3 className="text-lg font-medium text-[var(--text-primary)] ">Majors</h3>
 <p className="mt-1 text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">Manage majors within specific departments.</p>
 
 <div className="mt-6 flex flex-col sm:flex-row gap-4 items-end">
 <div className="flex-1 max-w-sm">
 <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)] ">Select Department</label>
 <select 
 ...
 <div className="p-8 text-center text-[var(--text-muted)]">No majors found in this department.</div>
 ) : (
 majors?.map((item) => (
 <div key={item.id} className={`flex items-center justify-between px-6 py-4 transition-colors ${!item.isActive ? 'opacity-60 bg-gray-50 /40' : ''}`}>
 <div className="flex items-center gap-2">
