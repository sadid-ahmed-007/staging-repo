...
function VerificationErrorPanel({ status, serial }) {
 ...
 const colorMap = {
 red: {
 wrap: 'bg-red-50 /20 border border-red-200 ',
 icon: 'text-red-500',
 title: 'text-red-800 ',
 message: 'text-red-700 ',
 hint: 'text-red-600/70 /70',
 },
 amber: {
 wrap: 'bg-amber-50 /20 border border-amber-200 ',
 icon: 'text-amber-500',
 title: 'text-amber-800 ',
 message: 'text-amber-700 ',
 hint: 'text-amber-600/70 /70',
 },
 };
 ...
 return (
 <div>
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-3">Verification Result</h3>
 ...
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] mt-2 font-mono">
...
export default function VerificationHistory() {
 ...
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] mt-1">{s.label}</p>
 </Card>
 ))}
 </div>
 )}
 ...
 {/* Status */}
 <div>
 <label className="block text-xs font-medium text-[var(--text-secondary)] dark:text-[var(--text-muted)] mb-1.5">Status</label>
 <select
 value={filters.status}
 onChange={(e) => setFilters((f) => ({ ...f, status: e.target.value }))}
 className="w-full rounded-xl border border-gray-300 bg-[var(--bg-surface)] px-3 py-2.5 text-sm text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 "
 >
 ...
 {/* Serial */}
 <div>
 <label className="block text-xs font-medium text-[var(--text-secondary)] dark:text-[var(--text-muted)] mb-1.5">Serial Number</label>
 ...
 className="w-full rounded-xl border border-gray-300 bg-[var(--bg-surface)] pl-9 pr-3 py-2.5 text-sm font-mono text-[var(--text-primary)] placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 dark:placeholder-gray-500"
 />
 </div>
 </div>
 {/* Date range */}
 <div className="sm:col-span-2">
 <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1.5">Date Range</label>
 <DateRangePicker
 ...
 <History className="h-8 w-8 text-gray-300 dark:text-[var(--text-secondary)]" />
 </div>
 <div>
 <p className="font-semibold text-[var(--text-primary)] ">No verifications found</p>
 ...
 <th key={h} className="py-3 px-4 text-left text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)] dark:text-[var(--text-muted)] first:pl-0 last:pr-0 last:text-right">
 {h}
 </th>
 ))}
 </tr>
 ...
 <span className="font-mono text-xs font-medium text-[var(--text-primary)] ">{row.serial}</span>
 </td>
 <td className="px-4 py-0">
 <span className="text-xs text-[var(--text-secondary)] ">{row.student_name || '—'}</span>
 </td>
 <td className="px-4 py-0">
 <span className="text-[var(--text-muted)] dark:text-[var(--text-muted)] text-xs">{row.institution || '—'}</span>
 </td>
 <td className="px-4 py-0">
 <StatusBadge status={row.status} />
 </td>
 <td className="px-4 py-0">
 <span className="text-[var(--text-muted)] dark:text-[var(--text-muted)] text-xs whitespace-nowrap">{row.verified_at}</span>
 </td>
 <td className="px-4 py-0 pr-0 text-right">
 <button
 onClick={() => setSelectedLog(row)}
 className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium text-[var(--text-secondary)] dark:text-[var(--text-muted)] hover:bg-gray-100 dark:hover:bg-gray-800 transition"
 >
 ...
 : 'border border-gray-200 text-[var(--text-secondary)] hover:bg-gray-100 dark:hover:bg-gray-800'
 }`}
 >
 {page}
 </button>
 ...
 <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">{selectedLog.details}</p>
 </div>
 </>
 );
 })()}
 ...
 <span className="text-xs text-[var(--text-muted)]">Verified: {selectedLog.verified_at}</span>
